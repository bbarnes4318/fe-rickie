...
export const runAmericanAmicableAutomation = async (data, jobId = null) => {
  ...
  const attemptRecoveryFlow = async (page, customerName) => {
    ...
    try {
      ...
      console.log(
        "[Recovery] Step 1: Looking for Save and Return to Applications button..."
      );
      ...
      const appFound = await page.evaluate((name) => {
        ...
        for (const link of links) {
          if (
            link.textContent &&
            link.textContent.includes(name.split(" ")[0])
          ) {
            link.click();
            return true;
          }
      ...
      try {
        ...
        const editClicked = await page.evaluate(() => {
          const btn =
            document.querySelector('input[value="Edit"]') ||
      ...
      try {
        const continueAppBtn = await page.$(
      ...
      console.log(
        "[Recovery] Step 6: Scrolling and clicking Continue button..."
      );
      await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
      await new Promise((r) => setTimeout(r, 1000));
      ...
      console.log(
        "[Recovery] Step 7: Clicking Continue to Agent Statement button..."
      );
      await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
      await new Promise((r) => setTimeout(r, 1000));

      try {
        const agentStmtBtn = await page.$(
          'input[value="Continue to Agent Statement"]'
        );
        if (agentStmtBtn) {
          await agentStmtBtn.click();
          console.log(
            "[Recovery] ✓ Clicked Continue to Agent Statement button"
          );
        } else {
          await page.evaluate(() => {
            const inputs = document.querySelectorAll('input[type="submit"]');
          ...
          console.log(
            "[Recovery] ✓ Clicked Continue to Agent Statement via JS"
          );
        }
      } catch (e) {
  ...
  console.log(
    `[PUPPETEER] ${logTs()} Customer: ${firstName} ${middleName} ${lastName}`
  );
  console.log(
  ...
  try {
    ...
    console.log(
      `[PUPPETEER] ${logTs()} Executable path: ${
        executablePath || "Using bundled Chrome"
      }`
    );
    ...
    try {
      ...
      console.log(
        "[Automation] Continue image not found, checking for link..."
      );
    ...
    try {
      ...
      console.log(
        "[Automation] Exact Mobile link not found, trying alternatives..."
      );
      try {
        ...
        console.log(
          "[Automation] Available links:",
          JSON.stringify(allLinks.slice(0, 10))
        );
      }
    }
    ...
    if (foundMobileLink) {
      ...
      await page
        .click(mobileLinkSelector)
        .catch(() => page.click(mobileLinkAlt));
      ...
      console.log(
        "[Automation] Navigating directly to insuranceapplication.com..."
      );
      await page.goto("https://www.insuranceapplication.com/", {
        waitUntil: "networkidle0",
        timeout: 60000,
    ...
    const mobileAppSelector =
      'a[href="https://www.insuranceapplication.com/cgi/webappmobile/"]';
    ...
    if (loginFormExists) {
      console.log(
        "[Automation] Login form detected, performing mobile login..."
      );
    ...
    try {
      ...
      const appLinks = await page.$$eval('a[href*="cgi/webappmobile"]', (els) =>
        els
          .filter(
            (a) =>
              !a.href.includes(".pdf") &&
              !a.href.includes("DocHandler") &&
              !a.href.includes("Demo")
          )
          .map((a) => a.href)
      );
      ...
      if (appLinks.length > 0) {
        ...
        try {
          ...
          console.log(
            "[Automation] ✓ Found exact webappmobile link, clicking..."
          );
      ...
      console.log(
        "[Automation] webappmobile link not found, logging available links..."
      );
      const links = await page.$$eval("a", (as) =>
        as.map((a) => ({ href: a.href, text: a.innerText.slice(0, 50) }))
      );
      console.log(
        "[Automation] Available links on page:",
        JSON.stringify(links.slice(0, 15))
      );
    ...
    if (retryMode) {
      ...
      console.log(
        "[Automation] Skipping new application - using recovery flow..."
      );
      ...
      const pendingClicked = await page.evaluate(() => {
        ...
        for (const link of links) {
          if (
            link.textContent &&
            link.textContent.toLowerCase().includes("pending")
          ) {
            link.click();
            return true;
          }
      ...
      if (recoverySuccess) {
        console.log(
          "[Automation] ✓ Recovery flow successful - now on Agent Statement page"
        );
        ...
        throw new Error(
          "Recovery flow failed - could not find or re-enter pending application"
        );
      }
      ...
      try {
        ...
        console.log(
          "[Automation] ProductMenu not found, checking for alternative popup..."
        );
        // Log visible popups
        const popups = await page.$$eval(
          '[id*="Menu"], [id*="Popup"], [class*="modal"]',
          (els) =>
            els
              .filter((e) => e.offsetParent !== null)
              .map((e) => ({ id: e.id, class: e.className }))
        );
      ...
      console.log(
        "[Automation] Product selection result:",
      ...
      console.log(
        "[Automation] State Elements:",
        JSON.stringify(pageDebug.stateElements)
      );
      console.log(
      ...
      if (stateDropdown) {
        ...
        try {
          ...
          console.log(
            "[Automation] page.select failed, trying to click option directly..."
          );
          ...
          if (optionClicked) {
            console.log(
              `[Automation] ✓ Selected "${state}" via direct option click`
            );
          } else {
        ...
        console.log(
          "[Automation] Looking for Submit button (BtnNewAppFinal)..."
        );
        ...
        console.log(
          "[Automation] #StateDropDown not found, trying alternative selectors..."
        );
        const altDropdown = await page.$(
          'select[name="StateDropDown"], select[name*="State"]'
        );
        if (altDropdown) {
          const stateCode = STATE_MAPPING[state];
          if (stateCode) {
            ...
            console.log(
              `[Automation] ✓ Selected state via alternative dropdown`
            );
      ...
      await page.click(
        `input[name="ctl00$ContentPlaceHolderMain$Sex"][value="${genderValue}"]`
      );
      ...
      else if (selectedPlanType === "Level" || selectedPlanType === "Immediate")
        planValue = "I";

      await page.click(
        `input[name="ctl00$ContentPlaceHolderMain$Plan"][value="${planValue}"]`
      );
      ...
      try {
        ...
        console.log(
          "[Automation] Continue button not found, quote may have different flow"
        );
      }
      ...
      try {
        ...
        console.log(
          "[Automation] Continue button not found after health questions"
        );
      }
      ...
      const effectiveBankCityState =
        bankCityState || `${city || "ANYTOWN"}, ${state || "IL"}`;
      const effectiveSSPaymentSchedule =
        ssPaymentSchedule !== null ? ssPaymentSchedule : false;
      ...
      console.log(
        "[Automation] effectiveAccountHolder:",
        effectiveAccountHolder
      );
      console.log("[Automation] effectiveBankName:", effectiveBankName);
      console.log(
        "[Automation] effectiveBankCityState:",
        effectiveBankCityState
      );
      console.log(
        "[Automation] effectiveSSPaymentSchedule:",
        effectiveSSPaymentSchedule
      );
      console.log("[Automation] effectiveDraftDay:", effectiveDraftDay);
      console.log(
        "[Automation] effectiveRoutingNumber:",
        effectiveRoutingNumber
      );
      console.log(
        "[Automation] effectiveAccountNumber:",
      ...
      try {
        ...
        console.log(
          "[Automation] ✓ Payment method set to Bank Draft (with events)"
        );
        await new Promise((r) => setTimeout(r, 2000)); // Wait for Bank Draft section to appear
      } catch (e) {
        console.log(
          "[Automation] ⚠ Payment method selector not found:",
          e.message
        );
      }
      ...
      try {
        ...
        if (bankField) {
          ...
          console.log(
            "[Automation] ✓ Bank Name entered:",
            effectiveBankName.toUpperCase()
          );
        } else {
      ...
      console.log(
        `[Automation] Using effectiveSSPaymentSchedule: ${effectiveSSPaymentSchedule}`
      );
      ...
      try {
        if (effectiveSSPaymentSchedule === true) {
          ...
          try {
            ...
            const hasValue = draftOptions.options.some(
              (o) => o.value === effectiveDraftDay
            );
            if (hasValue) {
              await page.select("#RequestedDraftDay", effectiveDraftDay);
              console.log(
                `[Automation] ✓ SS Draft Week selected: ${effectiveDraftDay}`
              );
            } else {
              console.log(
              ...
              const firstValue = draftOptions.options.find(
                (o) => o.value && o.value !== ""
              );
              if (firstValue) {
                await page.select("#RequestedDraftDay", firstValue.value);
                console.log(
                  `[Automation] ✓ Selected first available: ${firstValue.value}`
                );
              }
            }
          } catch (e) {
            ...
            if (firstSS)
              console.log(
                `[Automation] ✓ Fallback SS Week selected: ${firstSS}`
              );
          }
        } else {
          ...
          try {
            // First check if the value exists in options
            const hasValue = draftOptions.options.some(
              (o) => o.value === String(effectiveDraftDay)
            );
            if (hasValue) {
              await page.select(
                "#RequestedDraftDay",
                String(effectiveDraftDay)
              );
              console.log(
                `[Automation] ✓ Draft Day of Month selected: ${effectiveDraftDay}`
              );
            } else {
              console.log(
            ...
            console.log(
              `[Automation] Could not select day "${effectiveDraftDay}": ${e.message}`
            );
            ...
            if (firstDay)
              console.log(`[Automation] ✓ Fallback Day selected: ${firstDay}`);
          }
        }
        ...
        console.log(
          "[Automation] ✓ Final RequestedDraftDay value:",
          finalDraftValue
        );
      } catch (e) {
        console.log(
          "[Automation] SS Payment Schedule section error:",
          e.message
        );
      }
      ...
      try {
        ...
        if (routingField) {
          ...
          await page.type(
            "#TransitNumber",
            effectiveRoutingNumber.replace(/[^0-9]/g, "")
          );
          console.log(
            "[Automation] ✓ Routing Number entered:",
            effectiveRoutingNumber
          );
        } else {
      ...
      try {
        ...
        if (accountField) {
          ...
          await page.type(
            "#AccountNumber",
            effectiveAccountNumber.replace(/[^0-9]/g, "")
          );
          console.log(
            "[Automation] ✓ Account Number entered:",
            effectiveAccountNumber
          );
        } else {
      ...
      console.log(
        `[Automation] Using effectiveAccountType: ${effectiveAccountType}`
      );
      try {
        const checkPlanId =
          effectiveAccountType === "Saving" ||
          effectiveAccountType === "Savings"
            ? "CheckPlan_2"
            : "CheckPlan_1";
        const checkPlanLabel =
          checkPlanId === "CheckPlan_2" ? "Saving" : "Checking";
        ...
        console.log(
          `[Automation] ✓ Account Type: ${checkPlanLabel} (with events)`
        );
      } catch (e) {
        console.log(
          "[Automation] Checking/Savings radio not found:",
          e.message
        );
      }
      ...
      try {
        ...
        try {
          const feetInput = await page.$(
            '#HeightFeet, input[name*="HeightFeet"]'
          );
          const inchesInput = await page.$(
      ...
      try {
        ...
        console.log(
          "[Automation] Weight field #Weight not found, trying alternatives..."
        );
        // Try alternative selectors
        try {
          ...
          if (weightInput) {
            ...
            console.log(
              `[Automation] ✓ Weight entered via alt selector: ${safeWeight}`
            );
          } else {
            console.log(
              "[Automation] ✗ No weight field found with any selector"
            );
          }
        } catch (e2) {
      ...
      if (beneficiaryRelation) {
        try {
          ...
          console.log(
            `[Automation] ✓ Primary Beneficiary Relationship: ${radioId}`
          );
        ...
        try {
          ...
          console.log(
            "[Automation] ✓ Primary Beneficiary Relationship: Family Member (default)"
          );
          await new Promise((r) => setTimeout(r, 500));
          ...
          console.log(
            "[Automation] Could not set default beneficiary relationship"
          );
        }
      }
      ...
      try {
        ...
        console.log(
          "[Automation] Designee section search results:",
          JSON.stringify(designeeInfo)
        );
        ...
        if (!clicked) {
          ...
          clicked = await page.evaluate(() => {
            ...
            for (const el of allElements) {
              if (
                el.textContent &&
                el.textContent.trim() === "Will Not Designate"
              ) {
        ...
        if (!clicked) {
          ...
          try {
            ...
            if (labelElement) {
              ...
              console.log(
                "[Automation] ✓ Illinois Designee: Will Not Designate (via XPath)"
              );
              clicked = true;
            }
          } catch (e) {
      ...
      // ████████████████████████████████████████████████████████████████████████
      console.log(
        "[Automation] ═══ NOW VALIDATING BANK INFO (after page complete) ═══"
      );
      try {
        ...
        if (btnExists) {
          ...
          const validationResult = await page.evaluate(() => {
            ...
            return {
              ...
              isValidated:
                ...
                (msgDiv &&
                  msgDiv.textContent &&
                  msgDiv.textContent.includes("Successful")),
          ...
          console.log(
            "[Automation] Bank validation:",
            bankPassed ? "✓ SUCCESS" : "⚠ FAILED"
          );
          console.log(
            "[Automation] Validation message:",
          ...
          console.log(
            "[Automation] ⚠ Validate Bank Info button not found in DOM"
          );
        }
      } catch (e) {
        console.log(
          "[Automation] ⚠ Validate Bank Info error:",
          e.message,
          "- continuing anyway"
        );
      }
      ...
      // ═══════════════════════════════════════════════════════════════════════
      console.log(
        "[Automation] Looking for Continue to Agent Statement button..."
      );
      ...
      try {
        const pageState = await page.evaluate(() => {
          const result = {
            hasError:
              typeof HasError === "function"
                ? HasError()
                : "HasError not defined",
            isBankValidated:
              typeof IsValidatedBankInfo === "function"
                ? IsValidatedBankInfo()
        ...
        console.log(
          "[Automation]   - Red error elements found:",
          pageState.errorMessages.length
        );
        if (pageState.errorMessages.length > 0) {
          pageState.errorMessages.forEach((err, i) => {
            console.log(
              `[Automation]     ${i + 1}. [${err.tag}#${err.id}]: "${err.text}"`
            );
          });
        }
        console.log(
          '[Automation]   - Visible "Req" spans found:',
          pageState.allReqSpans.length
        );
      ...
      try {
        ...
        console.log(
          "[Automation] Attempting to click Continue to Agent Statement button..."
        );
        ...
        try {
          ...
          await page.evaluate(() => {
            ...
            spans.forEach(el => {
              ...
              if (text.includes('If selecting Trust') || 
            ...
            const errorElements = document.querySelectorAll('[style*="color: red"], [style*="color:red"]');
          ...
          if (!urlAfter.includes("personalinfo")) {
            ...
            console.log(
              "[Automation] Still on personalinfo, trying __doPostBack..."
            );

            await page.evaluate(() => {
              __doPostBack(
                "ctl00$ContentPlaceHolderBottomButton$BtnContinue",
                ""
              );
            });
        ...
        if (!buttonClicked) {
          console.log(
            "[Automation] ⚠ Continue to Agent Statement button not found"
          );
          ...
          console.log(
            "[Automation] After Continue click - URL:",
            currentUrl,
            "Title:",
            pageTitle
          );
          ...
          const stillOnSamePage = await page.evaluate(() => {
            ...
            const hasError =
              typeof HasError === "function" ? HasError() : false;
          ...
          if (stillOnSamePage) {
            ...
            if (!recoverySuccess) {
              console.log("[Automation] ✗ Recovery flow failed");
              throw new Error(
                "Form validation errors preventing navigation to Agent Statement"
              );
            }
        ...
        try {
          ...
          console.log(
            "[Automation] Recovery also failed:",
    ...
    try {
      ...
      const hasExistingIns =
        data.hasExistingInsurance === true || data.hasExisting === true;
      ...
      console.log(
        `[Automation] ✓ Existing Insurance: ${hasExistingIns ? "Yes" : "No"}`
      );

      // Is the proposed insurance intended to replace or change any existing life insurance or annuity?
      const willReplace =
        data.willReplaceExisting === true || data.willReplace === true;
      ...
      if (continueResult.success) {
        console.log(
          "[Automation] ✓ Clicked Continue to Signatures (onclick handler removed)"
        );
      } else {
        ...
        try {
          applicationNumber = await page.evaluate(() => {
            ...
            if (appNumberLabel) {
              ...
              const innerSpan = appNumberLabel.querySelector(
                'span[style*="x-large"]'
              );
          ...
          console.log(
            "[Automation] ✓✓✓ APPLICATION NUMBER CAPTURED:",
            applicationNumber
          );
        } catch (e) {
          console.log(
            "[Automation] Error capturing application number:",
            e.message
          );
        }
    ...
    console.log(
      "[Automation] Application Number:",
      applicationNumber || "(NOT CAPTURED)"
    );

    // CRITICAL: Only return success if we actually have an application number
    if (applicationNumber && applicationNumber.trim() !== "") {
      console.log(
        "[Automation] ✓✓✓ AUTOMATION COMPLETE - READY FOR VOICE RECORDING ✓✓✓"
      );
      console.log(
        "[Automation] ════════════════════════════════════════════════════════"
      );
      ...
      console.log(
        "[Automation] ✗✗✗ AUTOMATION FAILED - NO APPLICATION NUMBER CAPTURED ✗✗✗"
      );
      console.log(
        "[Automation] ════════════════════════════════════════════════════════"
      );
